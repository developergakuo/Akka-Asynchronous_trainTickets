...
object TSTravelService{
  ...
  class TravelService(routeService: ActorRef, trainService: ActorRef, trainTicketInfoService: ActorRef,
                      orderService: ActorRef, SeatService: ActorRef ) extends PersistentActor {
    ...
    override def receiveCommand: Receive = {
      ...
      case c: GetTripAllDetailInfo =>
        ...
        state.trips.get(c.gtdi.tripId) match {
          case Some(trip) =>
            ...
            if (response != null) {
              gtdr = TripAllDetail(response.get, trip)
            }
      ...
      case c:GetRouteByTripId =>
        println("TravelService: GetRouteByTripId " )
        state.trips.get(c.tripId) match {
          case Some(trip) =>
            getRouteByRouteId(trip.routeId) match {
              case Some(route) =>
                println("TravelService: GetRouteByTripId Success")
    ...
    def getTickets(trip: Trip, route: Route, startingPlaceId: Int, endPlaceId: Int, startingPlaceName: String,
      ...
      if (travelResultResponse.status == 0) {
        ...
        val soldTicketResponseFuture: Future[Any] = orderService ? QueryAlreadySoldOrders(travelDate = departureTime, trainNumber = trip.tripId)
