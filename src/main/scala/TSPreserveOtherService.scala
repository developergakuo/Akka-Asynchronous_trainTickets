...
object TSPreserveOtherService {
  ...
  class PreserveOtherService (ticketInfoService: ActorRef,
                              securityService: ActorRef, contactService:  ActorRef, travel2Service: ActorRef, stationService: ActorRef, seatService: ActorRef,
                              orderOtherService: ActorRef, assuranceService: ActorRef,
    ...
    override def receive: Receive = {
      case Preserve(oti: OrderTicketsInfo2) =>
        //checkOrder security
        println("Begin lowSpeedOrder Preservation")
        if (!checkSecurity(oti.accountId)) {
          sender() ! Response(1, "Security Error 1", None)
        }
        else {
          //get contacts
                        ...
                        case Some(ord) =>
                          ...
                          if (!(oti.assurance == 0)) { //insure
                            ...
                            if (oti.foodType != 0) {
                             ...
                             println("suggested food order id: "+ ord.id)
                              if (oti.foodType == 2) {
    ...
    def getTripAllDetailInformation(gtdi: TripAllDetailInfo): Option[TripAllDetail]={
      ...
      val responseFuture: Future[Any] = travel2Service ? GetTripAllDetailInfo(gtdi)
      val response = Await.result(responseFuture,duration).asInstanceOf[Response]
    ...
    def createOrder(order: Order): Option[Order] ={
      ...
      val responseFuture: Future[Any] = orderOtherService ? Create(order)
      val response = Await.result(responseFuture,duration).asInstanceOf[Response]
    ...
    def addAssuranceForOrder(assuranceType:Int, orderId:Int):Option[Int]={
      ...
      val response = Await.result(responseFuture,duration).asInstanceOf[Response]
      if(response.status ==0) assuranceResp = Some(0)
      assuranceResp
    }
    ...
    def createFoodOrder(foodOrder: FoodOrder): Option[Int] ={
      ...
      val response = Await.result(responseFuture,duration).asInstanceOf[Response]
      if (response.status == 0) {
        println("Food order creation success")
        foodOrderResp = Some(0)
      } else println("Food-order creation error")
      foodOrderResp
    }
