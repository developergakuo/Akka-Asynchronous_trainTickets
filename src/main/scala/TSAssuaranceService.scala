 ...
 object TSAssuranceService {
   ...
   case class AssuranceService(assurances: Map[Int, Assurance])

   class OrderService extends PersistentActor {
     var state: AssuranceService = AssuranceService(Map())
     ...
     override def receiveRecover: Receive = {
       case SnapshotOffer(_, offeredSnapshot: AssuranceService) ⇒ state = offeredSnapshot
     ...
     def updateState(evt: Evt): Unit = evt match {
       case c: CreateAssurance2 ⇒
         ...
         state = AssuranceService(state.assurances + (id -> Assurance(id, c.orderId, c.assuranceType)))
       case c: DeleteById =>
         state = AssuranceService(state.assurances - c.assuranceId)
     }
